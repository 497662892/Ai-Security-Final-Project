...
class PolypDataser_csv(data.Dataset):
    ...
    dataloader for polyp segmentation tasks
    """
    def __init__(self, csv_root, trainsize, augmentations, switch_ratio = 0.0, 
                 align_score_cutoff=0.8, max_aug=None, selection_rule="random", use_background=False):
        # read the csv file
        csv_file = pd.read_csv(csv_root)
        # get the images and the masks path of the original
        self.images = csv_file['original_image'].tolist()
        self.gts = csv_file['original_mask'].tolist()
        ...
        if not max_aug:
            self.images += filter_csv['inpaint_image'].tolist()
            self.gts += filter_csv['inpaint_mask'].tolist()
            
            if use_background:
                self.images += filter_csv['inpaint_background'].tolist()
                self.gts += ["negative"] * len(filter_csv['inpaint_background'].tolist()) 
            ...
            elif selection_rule == "prediction_score":
                sorted_data = filter_csv.sort_values(by='prediction_score', ascending=True)
                sample_number = min(max_aug, len(filter_csv))
                sample_csv = sorted_data.iloc[:sample_number]
                self.images += sample_csv['inpaint_image'].tolist()
                self.gts += sample_csv['inpaint_mask'].tolist()
                
            elif selection_rule == "priority":
                sorted_data = filter_csv.sort_values(by='priority', ascending=True)
                sample_number = min(max_aug, len(filter_csv))
                sample_csv = sorted_data.iloc[:sample_number]
                self.images += sample_csv['inpaint_image'].tolist()
                self.gts += sample_csv['inpaint_mask'].tolist()
            ...
            if use_background:
                self.images += sample_csv['inpaint_background'].tolist()
                self.gts += ["negative"] * len(sample_csv['inpaint_background'].tolist())
...
def get_updated_loader(image_root, gt_root, subfolders, batchsize, trainsize, shuffle=True, switch_ratio = 0, num_workers=4, pin_memory=True, augmentation=False,
                       sampler = None, training_type = 'original', align_score_cutoff = 0.8, max_aug = None, selection_rule = "random", use_background=False):
    if training_type == 'original':
        dataset = PolypDataset(image_root, gt_root, subfolders, trainsize, augmentation, switch_ratio=switch_ratio)
    else:
        dataset = PolypDataser_csv(image_root, trainsize, augmentation, switch_ratio=switch_ratio, 
                                     align_score_cutoff=align_score_cutoff, max_aug=max_aug, selection_rule=selection_rule, use_background=use_background)
